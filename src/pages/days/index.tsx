...
import { getDays, deleteDayById } from 'apiSdk/days';
import { DayInterface } from 'interfaces/day';

type ColumnType = ColumnDef<DayInterface, unknown>;

interface DayListPageProps {
...
export function DayListPage(props: DayListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getDays({
        relations: ['calendar', 'meeting.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['settings.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DayInterface>>(
    () => `/days?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DayInterface) => {
    if (hasAccess('day', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/days/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'settings', header: 'Settings', accessorKey: 'settings' },
    hasAccess('calendar', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/calendars/view/${record.calendar?.id}`}>
    ...
    hasAccess('meeting', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Meeting',
          accessorKey: 'meeting',
          cell: ({ row: { original: record } }: any) => record?._count?.meeting,
        }
      : null,

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/days/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('day', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/days/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('day', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="day" />
        </Flex>

        {hasAccess('day', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/days/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
