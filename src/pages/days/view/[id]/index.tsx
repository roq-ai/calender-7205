...
import { getDayById } from 'apiSdk/days';
...
function DayViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<DayInterface>(
    () => (id ? `/days/${id}` : null),
    () =>
      getDayById(id, {
        relations: ['calendar'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('day', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/days/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Settings" text={data?.settings} />
                ...
                {hasAccess('calendar', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Calendar"
                    text={
                      <Link as={NextLink} href={`/calendars/view/${data?.calendar?.id}`}>
                        {data?.calendar?.year}
                      </Link>
                    }
                  />
