...
import { getDayById, updateDayById } from 'apiSdk/days';
...
function DayEditPage() {
  ...
  const handleSubmit = async (values: DayInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDayById(id, values);
      mutate(updated);
      resetForm();
      router.push('/days');
    } catch (error) {
  ...
  const formik = useFormik<DayInterface>({
    initialValues: data,
