...
type ColumnType = ColumnDef<MeetingInterface, unknown>;
...
export function MeetingListPage(props: MeetingListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MeetingInterface>>(
    () => `/meetings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('day', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'day',
          header: 'Day',
          accessorKey: 'day',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/days/view/${record.day?.id}`}>
              {record.day?.settings}
            </Link>
          ),
        }
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/meetings/view/${record.id}`} passHref legacyBehavior>
            <Button
